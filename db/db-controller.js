...
var getAllQuests = function(characterId) {
  return new Promise(function(resolve, reject) {
    return connection.queryAsync('SELECT q.id, q.name, q.creator_id, q.lat, q.lng, q.questType, q.complete, q.creator_lat, q.creator_lng, q.timestamp, q.timestart, q.timestop, q. c.active FROM Quests q LEFT OUTER JOIN CharacterQuests c ON (q.id = c.quest_id AND c.character_id = ' + characterId +')').then(function(result) {
